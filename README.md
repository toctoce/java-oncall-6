...
- [ ] 입력 : 숫자, 요일
- [ ] 연도는 없음, 윤년도 없음
- [ ] 올바르지 않은 입력은 [ERROR] 표시 후 다시 입력
  - [ ] 근무 순번 입력이 잘못되었다면 평일 순번부터 다시 입력
- [ ] 평일이면서 법정공휴일일 때만 요일 표기하기
- [ ] 출력 후 프로그램 종료
...
- order
  ...
  - [ ] 두 순번 입력으로 생성자
  - [ ] 각 입력 중복 닉네임 체크
  - [ ] 두 입력들이 같은 사람으로 이루어져있는지 체크
  - [ ] 5명 이상 35명 이하인지 체크
- date
  - [ ] 월, 일, 요일, 공휴일 여부를 가지고 있는 레코드
  - [ ] 다음날
- schedule
  - [ ] date, order로 스케줄 생성
    - [ ] 1차 순번 생성
